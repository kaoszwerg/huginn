...
ADR-030 pins the portable governance core and gates drift. In practice the gate fired on work a
...
- **Portable core (pinned).** Files that encode governance or a gate every project inherits unchanged:
  ...
  below. Unchanged from ADR-030: in-place edits in a fork are fatal.
...
- ADR-033 (layered governance — "core" here reads as "any upstream-owned layer"), ADR-030 (superseded),
  ADR-007 (hash gate), ADR-009/ADR-011 (dependency + security exceptions),
