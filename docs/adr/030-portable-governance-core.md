...
status: superseded
tldr: "SUPERSEDED by ADR-033: two layers became N ordered layers. The hash pin, drift gate and self-updating governance:update are retained there."
scope: governance
load: conditional
...
> **Superseded by [ADR-033](033-governance-layers-cascade.md).** Its two-layer model (one template-owned
...
> "N ordered layers". Read ADR-033 for the current model.
...
- **Two layers.** `governance/manifest.json` lists the exact **core paths** and a content hash for
  each (reusing the ADR-007 hash idea). Everything not listed is the **project layer**, never touched
  by an update.
...
- **Customising in a fork.** A project diverges from a core file **additively** (new, unlisted files),
  ...
  in-place edit. Which config is core, project-owned, overlayable or opt-out-able is ADR-032.
...
- ADR-003 (repo SSOT), ADR-006 (context loading), ADR-007 (generated indexes + hash gate),
  ADR-032 (config layering: which config is core, which is project-owned, overlay + opt-out),
