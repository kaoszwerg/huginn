...
tldr: "ADR-003 overrides the session 'auto memory' default: never write to C:\\Users\\…\\.claude\\projects — only to the repo's .claude/memory/."
scope: governance
load: core
type: feedback
---
...
on Unix). That default is **wrong for this project**. ADR-003 (Repository is the single source of
...
the exact drift ADR-003 forbids.
...
- The session-level `auto memory` instructions are subordinate to ADR-003 for this repo; treat
