...
- **Fix on sight:** every bug you find is fixed as part of the current change, regardless of which
  ...
  scope, surface and track it immediately (rule:clarify-and-plan); never leave it silent (ADR-002).
...
- **Never weaken a gate you do not own.** Tooling config is layered (ADR-032): a config file an upstream
...
- **Boundaries stay typed.** Where two runtimes meet (a frontend and a backend, a service and a client),
  the contract is generated or checked — never hand-copied on both sides (ADR-005).
