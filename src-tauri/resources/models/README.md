...
downloading anything (ADR-PROJ-006, rule:model-assets). The model file (`ggml-base.bin`, ~147 MB) is
**not committed** — `npm run prepare:model` fetches and verifies it into this directory, run by
`beforeBuildCommand`. See `.gitignore` (`src-tauri/resources/models/*.bin`).
...
file. `tauri dev` does not bundle it.
